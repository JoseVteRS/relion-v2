...
import { ItemStatus, ItemVisibility, Present } from "@prisma/client";
...
export const PresentEditForm = ({ present }: PresentEditFormProps) => {
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      externalLink: present.externalLink || "",
    },
  });
  ...
  const createPresentMutation = useMutation(
    trpc.present.edit.mutationOptions({
      onSuccess: (present) => {
        navigate({ to: "/dashboard/presents" });
        toast.success(`Regalo editado correctamente: ${present.name}`);
      },
      onError: (error) => {
        toast.error("Error al editar el regalo");
      },
    })
  );
