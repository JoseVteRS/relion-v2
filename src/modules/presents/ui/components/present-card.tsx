...
export const PresentCard = ({ present }: PresentCardProps) => {
  const { 
    handlePick, 
    handleUnpick, 
    handleMarkAsBought,
  ...
  const cardVariant = {
    "UNPICKED": "",
    "PICKED": "border-amber-300 bg-amber-50 dark:bg-amber-950/30 bg-stripe-picked",
    "BOUGHT": "border-green-300 bg-green-50 dark:bg-green-950/30",
  }[present.pickedStatus];

  // Badge para mostrar el estado
  const statusBadge = {
    "UNPICKED": <Badge variant="outline" className="flex gap-1 items-center"><Gift className="h-3 w-3"/>Disponible</Badge>,
    "PICKED": <Badge className="flex gap-1 items-center bg-amber-500"><ShoppingBag className="h-3 w-3"/>Reservado</Badge>,
    "BOUGHT": <Badge className="flex gap-1 items-center bg-green-500"><CheckCircle2 className="h-3 w-3"/>Comprado</Badge>,
  }[present.pickedStatus];

  return (
    <>
      <Card className={cn("transition-all hover:shadow-md", cardVariant)}>
                ...
                <div className="text-sm text-amber-600 dark:text-amber-400 flex items-start gap-2 mt-1 p-2 bg-amber-50 dark:bg-amber-950/30 rounded-md">
                  ...
                  <div>
                    ...
                    <Button variant="link" asChild className="px-1 h-auto">
                      <a href="/auth/register" className="font-bold">Regístrate</a>
                    </Button>
                    para elegir más.
                  </div>
                </div>
